...
class ExplainerClassifierModel(pl.LightningModule):
    def __init__(self, classifier, num_classes=14, dataset="lang_data", learning_rate=1e-5, class_mask_min_area=0.05, 
                 class_mask_max_area=0.3, entropy_regularizer=1.0, use_mask_variation_loss=False, mask_variation_regularizer=1.0, use_mask_area_loss=False, 
    ...
    def forward(self, image, targets):
        ...
        target_mask = (image > image.mean()).int()
        inversed_target_mask = torch.ones_like(target_mask) - target_mask
        non_target_mask  = (image <= image.mean()).int()
        
        masked_image = target_mask * image
        inversed_masked_image = inversed_target_mask * image
